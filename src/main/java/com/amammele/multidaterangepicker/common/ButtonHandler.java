package com.amammele.multidaterangepicker.common;
...
import com.amammele.multidaterangepicker.MultiDateRangePicker;
...
public class ButtonHandler implements View.OnClickListener {
    ...
    public ButtonHandler(@NonNull MultiDateRangePicker multiDateRangePicker) {
        Context context = multiDateRangePicker.getContext();
        ...
        if (mIsInLandscapeMode) {
            initializeForLandscape(multiDateRangePicker);
        } else {
            // Takes care of initialization
            mPortraitButtonHandler = (ButtonLayout) multiDateRangePicker.findViewById(R.id.button_layout);
        }
    }

    private void initializeForLandscape(MultiDateRangePicker sublimeMaterialPicker) {
        Context context = SUtils.createThemeWrapper(sublimeMaterialPicker.getContext(),
                R.attr.mdPickerStyle,
                R.style.MDPickerStyleLight,
                R.attr.mdButtonLayoutStyle,
        ...
        try {
            ...
            // buttons or icons?
            int presentation = a.getInt(R.styleable.ButtonLayout_mdPresentation, 0);

            int bgColor = a.getColor(R.styleable.ButtonLayout_mdButtonBgColor,
                    SUtils.COLOR_BUTTON_NORMAL);
            int pressedBgColor = a.getColor(R.styleable.ButtonLayout_mdButtonPressedBgColor,
                    SUtils.COLOR_CONTROL_HIGHLIGHT);

            mButtonBarBgColor = a.getColor(R.styleable.ButtonLayout_mdButtonBarBgColor,
                    Color.TRANSPARENT);

            int buttonInvertedBgColor =
                    a.getColor(R.styleable.ButtonLayout_mdButtonInvertedBgColor,
                            SUtils.COLOR_ACCENT);
            int buttonPressedInvertedBgColor =
                    a.getColor(R.styleable.ButtonLayout_mdButtonPressedInvertedBgColor,
                            ContextCompat.getColor(context, R.color.md_ripple_material_dark));
            ...
            if (presentation == 0 /* mode: Button */) {
                ...
                mIconOverlayColor = a.getColor(R.styleable.ButtonLayout_mdIconColor,
                        SUtils.COLOR_ACCENT);
    ...
    public void updateSwitcherText(@NonNull MultiDateOptions.Picker displayedPicker, CharSequence text) {
        if (mIsInLandscapeMode) {
            if (displayedPicker == MultiDateOptions.Picker.DATE_PICKER) {
                mSwitcherButtonDP.setText(text);
            } else if (displayedPicker == MultiDateOptions.Picker.TIME_PICKER) {
